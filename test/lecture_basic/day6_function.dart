...
// 전역 변수 : 프로그램이 종료될 때까지 존재 => 데이터 영역에 저장됨
int global = 10; //<-- 전역 변수
void Function(int x) func() {
  // 지역 변수 : 함수가 실행되는 동안 존재 => 스택 영역에 저장됨
  int local = 10; // <-- 지역 변수

  // 내부 함수 : 함수 내부에 선언된 함수를 내부 함수 라고 한다.
  // 특징
  // 내부 함수는 외부 함수의 변수를 참조 할 수 있다. 이때, 외부 함수의 변수를 외부 변수(outer variable)라고 한다.
  void inner() {
    // 지역 변수 : 함수가 실행되는 동안 존재 => 스택 영역
    int innerLocal = 10; // 지역변수임
  ...
  // innerLocal = 0; // 불가능 // 내부함수의 변수를 외부 함수에서 꺼내 쓸 수는 없다.
  local = 0;
  global = 0; // 전역변수는 어디든 사용 가능하다.
...
/*
클로저(Closure):
    외부 함수의 변수를 사용하는 내부 함수나 익명 함수는 해당 변수를 포착(capture)하여 클로저를 형성합니다.
    클로저는 외부 변수를 계속 참조하여 그 변수의 값에 접근할 수 있습니다.
    클로저를 사용할 때는 메모리 누수(memory leak)가 발생할 수 있으므로 주의가 필요합니다.
 */