...
void main() {
  ...
  final bool isFall = x == FALL;// final : 이후 값을 바꾸지 않을 때 => 결과물 상수화
  if ( isFall ) { /* ... */ }
  ...
  // // if-else 중 하나를 실행해야하지만, 부가코드도 실행을 해야할 때 유리
  // if() { /*...*/ }
  // else { /*...*/ }
  // /* 부가코드 */ // 여기도 실행됨
  //
  // // 함수에서 유리함, 코드 가독성도 높아짐, 중단점 => 범위축소 => 디펜스 코드
  // if() { /*...*/ return; }
  // /* ... */
  // /* 부가코드 */ // 여기도 실행됨
  ...
  switch(x) {
    ...
    default:
      ...
      // message에 대한 기본 값 처리
      if(message?.isEmpty ?? true) { // 메시지가 없었을 경우 빈 값인 것 처럼 취급
