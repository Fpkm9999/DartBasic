/**
 * - 배열(Array)
 *   같은 데이터 타입을 N개 생성한 것 > 메모리 공간 N개를 일렬(연속적)로 선언한 것
 *
...
void main() {
  ...
  // 바로 위처럼 하면 값 복사로 수행됨(깊은복사 Deep Copy)
  ...
  // 상수화 - 리스트를 가리키는 변수의 레퍼런스 주소에 대해서 상수화 (리스트 내부는 변경 가능한 상태)
  final constants1 = [];
  var constants2 = List.unmodifiable(initializedList2);// 내용물 상수화
  constants2 = [1,2,3,4];
  const constList = const[ 1,2,3,4 ];// final과의 차이 : list가 컴파일 타임부터 상수화됨
  ...
  print("현재 배열 상태1 : (${initializedList1.length})");// 현재 배열 상태2 : (11)
  initializedList1.removeAt(10); // 0~10까지만 접근 가능한 상태
  print("현재 배열 상태2 : (${initializedList1.length})");// 현재 배열 상태2 : (10)
  print("9번째 : ${ initializedList1[9] }");
  initializedList1.clear();// 모두 지우기
  bool isEmpty = initializedList1.isEmpty; // 리스트가 비었는가?
  bool isNotEmpty = initializedList1.isNotEmpty; // 리스트가 비어있지 않은가?
