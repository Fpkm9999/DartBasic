...
void main() {
  ...
  print(sum);
  List<int> list2 = [for (int i = 0; i < 10; i++) i];
  print(list2);

  // 평균 값
  // int/double 일 때 (num 타입) 나누게되면, double 타입 결과물
  // => 자연스러움, WHY? 소수점도 데이터라서 중요함
  // 그 결과물을 int로 얻고 싶었다 => toInt() 호출하게 될 거임
  // => 축약 문법으로 나눈 후 정수화('~/') 연산자 사용
  int average = sum ~/ list.length; // (sum / list.length).toInt(); 와 같음
  print(average);

  // N의 배수 : https://school.programmers.co.kr/learn/courses/30/lessons/181937
  // 문제 변형 : N의 배수를 20개를 배열에 담아서 출력 => 당연히 중복 없음
  ...
  for (int i = 1; i <= 9; i++) {
    ...
    // 중첩 for 문 : 바깥 for문이 1번 수행될 때, 안쪽 for문은 N번 수행
    for (int j = 1; j <= 9; j++) {
      // 1X1
      // 1X2
      // 1X3 ...
  ...
  int rand2 = random.nextInt(pow(2, 32).toInt()) % 1001;
  //위의 값과 범위가 같음
  // 범위 조정 시 알아야할 규칙
  // 나머지('%') 연산자의 규칙 : 나누는 수(N)를 넘지 않음 => 0 ~ N-1
  // 나머지 연산자에 덧셈('+') 연산자 사용 : 쉬프팅 연산이 됨
  //                                    (A+S ~ B+S)
  rand1 = rand1.abs(); // 무조건 양수로 만들기 => 절다값 abs 함수
  ...
  const int row = 5;
  const int column = 5;
  var matrix = List.generate( row,// 1개가 있을 때
        (index) => List.generate(
      column, // y개 들어 있음
          (index) => 0,
    ),
